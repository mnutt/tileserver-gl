...
describe('FontManager', () => {
  ...
  describe("getFontPbf()", () => {
    ...
    it ('returns fallback pbf data for an unknown font', async () => {
      const pbf = await fontManager.getFontPbf('Comic Sans Bold', '22272-22527', ['Open Sans Italic', 'Open Sans Bold']);
    ...
    it ('throws error if no suitable fallbacks exist', async () => {
      const result = fontManager.getFontPbf('Comic Sans Bold', '22272-22527', ['Marker Bold', 'Comic Sans Regular']);
      should(result).be.rejectedWith('Font load error: Comic Sans Bold');
    });

    describe("getFontsPbf()", () => {
      it("returns composed glyphs", async () => {
        const fonts = await fontManager.getFontsPbf('Open Sans Bold, Open Sans Italic, Open Sans Regular',
                                                    '22272-22527');
