...
class StyleManager {
  ...
  get fontsList() {
    return Object.values(this.styles)
      .map((style) => style.fontList)
      .reduce(flatten, []);
  }

  get sourcesList() {
    return Object.values(this.styles)
      .map((style) => style.sourceList)
      .reduce(flatten, []);
  }
  ...
  async parse(item, id) {
    ...
    const sourceList = Object.entries(styleJSON.sources).map(([name, source]) => {
      return { name, source, mapping: item.mapping };
    });
